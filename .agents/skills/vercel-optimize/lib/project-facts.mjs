...
export function deriveProjectFacts(signals) {
  ...
  if (cfg.functionDefaultMemoryType === 'standard') {
    out.push({
      id: 'memory_standard',
      strength:
        'Function memory tier: Standard (2GB) — the cost-efficient default; upgrade to Performance (4GB) only if you have evidence of memory-bound routes.',
      briefLine:
        'Function memory tier is Standard (2GB), the cost-efficient default. Recommending an upgrade to Performance (4GB) requires memory-saturation evidence.',
      contradictPhrases: [],
    });

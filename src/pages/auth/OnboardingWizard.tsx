...
export default function OnboardingWizard() {
  ...
  const steps: Step[] = [
    { label: 'Welcome', status: step > 0 ? 'done' : step === 0 ? 'current' : 'pending' },
    {
      label: 'Organization',
      status: step > 1 ? 'done' : step === 1 ? 'current' : 'pending',
    },
    { label: 'Data', status: step > 2 ? 'done' : step === 2 ? 'current' : 'pending' },
    { label: 'Done', status: step > 3 ? 'done' : step === 3 ? 'current' : 'pending' },
  ];
          ...
          >
            {step === 0 && (
              <div className="text-center space-y-4">
                <p className="text-slate-400" id="step-title-0">
                ...
                <div className="bg-slate-800 rounded-lg p-4 text-left space-y-2">
                  ...
                  <ul className="text-sm text-slate-400 space-y-1">
                    <li className="flex items-center gap-2">
                      <Building2 className="h-3.5 w-3.5" aria-hidden="true" /> Organization profile
                    </li>
                    <li className="flex items-center gap-2">
                      <Globe className="h-3.5 w-3.5" aria-hidden="true" /> Currency & fiscal year
                    </li>
                    <li className="flex items-center gap-2">
                      <Database className="h-3.5 w-3.5" aria-hidden="true" /> Financial data import
                    </li>
                  </ul>
                </div>
                <Button onClick={() => setStep(1)} className="w-full">
                  Get Started <ChevronRight className="h-4 w-4 ml-1" aria-hidden="true" />
                </Button>
              </div>
            )}
            ...
            {step === 1 && (
              <div className="space-y-4">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="font-semibold" id="step-title-1">
                    Organization Details
                  </h2>
                </div>
                <div className="space-y-3">
                  <div>
                    <label className="text-sm font-medium text-slate-300" htmlFor="company-name">
                      Company Name *
                    </label>
                    <input
                      id="company-name"
                  ...
                  <div>
                    <label className="text-sm font-medium text-slate-300" htmlFor="industry-select">
                      Industry
                    </label>
                    <select
                      id="industry-select"
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      >
                        <Globe className="h-3.5 w-3.5 inline mr-1" aria-hidden="true" />
                        Currency
                      </label>
                      <select
                        id="currency-select"
                    ...
                    <div>
                      <label className="text-sm font-medium text-slate-300" htmlFor="fy-start">
            ...
            {step === 2 && (
              <div className="space-y-4">
                <h2 className="font-semibold" id="step-title-2">
            ...
            {step === 3 && (
              <div className="text-center space-y-4">
                <CheckCircle2 className="h-14 w-14 text-green-400 mx-auto" aria-hidden="true" />
                <h2 className="font-semibold text-lg" id="step-title-3">
                  You&apos;re All Set!
                </h2>
                ...
                <div className="bg-slate-800 rounded-lg p-4 text-left">
                  ...
                  <ul className="text-sm text-slate-400 space-y-1">
                    <li>
                      • Press{' '}
                      <kbd className="px-1.5 py-0.5 bg-slate-700 rounded text-xs">Ctrl+K</kbd> to
                      open command palette
                    </li>
                    <li>
                      • Press{' '}
                      <kbd className="px-1.5 py-0.5 bg-slate-700 rounded text-xs">Ctrl+S</kbd> to
                      save
                    </li>

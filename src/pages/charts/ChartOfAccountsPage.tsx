...
import { BookOpen, DollarSign, Layers, TrendingUp } from 'lucide-react';
import type { GLEntry } from '@/types';
import { addMoney, roundTo, sumMoney } from '@/utils/money';
import { PageHeader } from '@/components/ui/PageHeader';

function computeChartStats(entries: readonly GLEntry[]) {
...
export function ChartOfAccountsPage() {
  ...
  const stats = useMemo(() => computeChartStats(entries), [entries]);
  ...
  if (entries.length === 0) {
    return (
      <main className="p-12 text-center" role="main" aria-label="Chart of Accounts - No Data">
        <a
          href="#import-btn"
        ...
        <h1 className="text-xl font-semibold mb-2">No Chart of Accounts Data</h1>
        <p className="text-[var(--text-muted)] mb-6">Import GL data to view chart of accounts.</p>
        <Button
          id="import-btn"
          onClick={() => navigate('/data/gl-upload')}
          onKeyDown={handleImportKeyDown}
          aria-label="Import GL data to view chart of accounts"
        >
          Import Data
        </Button>
      </main>
  ...
  return (
    <main
      ...
      role="main"
      aria-label="Chart of Accounts Dashboard"
    >
      <a
        href="#kpi-section"
        className="sr-only focus:not-sr-only focus:absolute focus:top-2 focus:left-2 focus:z-50 focus:px-4 focus:py-2 focus:bg-blue-600 focus:text-white focus:rounded"

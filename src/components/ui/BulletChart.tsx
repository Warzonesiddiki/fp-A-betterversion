    ...
    if (error) {
      return (
        <div className={cn('w-full', className)}>
          <div className="flex items-center justify-center h-12 text-red-600 text-sm"> {error}</div>
        </div>
      );
    }
    if (isNaN(actual) || isNaN(target))
      return (
        <div className="flex items-center justify-center h-12 text-slate-400 dark:text-slate-300">
          Invalid data
        </div>
      );

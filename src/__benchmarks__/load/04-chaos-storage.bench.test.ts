...
function detectHardware() {
  ...
  return { cpu: cpus[0]?.model ?? 'unknown', ram: `${totalMemMB}MB`, os: `${process.platform} ${process.arch}`, node: process.version };
}
...
describe('Vulcan — Chaos 01: LocalStorage Quota Exhaustion', () => {
  ...
  it('PHASE 1: fill localStorage incrementally until QuotaExceeded', () => {
    ...
    console.log(`[VULCAN] Chaos01-PHASE1: wrote ${written}KB in ${elapsed.toFixed(2)}ms, failed at: ${firstFailure ?? 'none'}`);
    ...
    records.push({
      ...
      userNotified: false,
      dataLoss: false,
      passed: firstFailure === 'QuotaExceededError' || firstFailure === 'NS_ERROR_DOM_QUOTA_REACHED',
  ...
  it('PHASE 2: attempt critical save when quota is full', () => {
    ...
    console.log(`[VULCAN] Chaos01-PHASE2: critical save error: ${caughtError ?? 'none'}, recovery: ${recoveryTimeMs.toFixed(2)}ms, userNotified: ${userNotified}, dataLoss: ${dataLoss}`);
  ...
  it('PHASE 3: storage.worker.stringify stress (1MB+ payloads)', () => {
    if (typeof Worker === 'undefined') {
      console.log('[VULCAN] Chaos01-PHASE3: Worker unavailable in node env — measure JSON.stringify CPU only');
      // CPU-only fallback
      const sizes = [100_000, 500_000, 1_000_000, 5_000_000];
      const results: { bytes: number; ms: number }[] = [];
      for (const n of sizes) {
        const data = { records: new Array(n / 100).fill({ x: Math.random(), y: 'string content '.repeat(5) }) };
        const start = performance.now();
        const json = JSON.stringify(data);
        const parsed = JSON.parse(json);
        const elapsed = performance.now() - start;
        results.push({ bytes: json.length, ms: Math.round(elapsed * 100) / 100 });
        console.log(`[VULCAN] stringify+parse ${n} records → ${(json.length / 1024).toFixed(1)}KB in ${elapsed.toFixed(2)}ms`);
      }
      const outDir = path.resolve(__dirname, '../../../tests/load');
      if (!fs.existsSync(outDir)) fs.mkdirSync(outDir, { recursive: true });
      fs.writeFileSync(path.join(outDir, '.raw-chaos-storage-cpu.json'), JSON.stringify({ hardware: hw, results }, null, 2));
      return;
    }
  });
  ...
  afterAll(() => {
    ...
    console.log(`[VULCAN] Wrote ${records.length} chaos-storage records to .raw-chaos-storage.json`);
  });
});

...
| Removed the duplicated `if: always()` key on the `summary` job                                                              | `ci.yml` is **invalid YAML** (`duplicated mapping key`, line 327). See the severity note below — this is the highest-impact line in the patch, not a tidy-up.                                                                                                                                                                    |

## Severity note (measured 2026-08-19, session 026)

The duplicate key is not cosmetic. GitHub rejects the whole workflow file, so
every `ci.yml` run **completes as `failure` with zero jobs**:

```
$ gh run view 32217442362
X arena/01a0182b-fp-a-betterversion .github/workflows/ci.yml
X This run likely failed because of a workflow file issue.

$ gh api .../actions/runs/32217442362/jobs --jq '.jobs[]|.name'
(empty)
```

Confirmed identical on `arena/01a0178d-...` (session 025) and on `main`, so it is
not branch-specific. Consequence: **`typecheck`, `lint`, `test` (the 4-way sharded
Vitest), `test-merge`, `build`, `e2e`, `a11y` and the blocking `CI Summary` gate
defined in `ci.yml` have never executed.** The checks that do run on a PR come
from the separate single-purpose workflows (`tsc.yml`, `lint.yml`, `build.yml`,
`test-unit.yml`, `cascade-hold-check.yml`, `sentry-self-test.yml`), which is why
this went unnoticed — the PR page looks well covered.

Because `ci.yml` emits no check runs, it never appears in `gh pr checks`, and
because it is red for a _parsing_ reason its failure carries no diagnostic. Do
not read a red `ci.yml` as a test failure, and do not read a green PR page as
evidence that `ci.yml` ran. Applying this patch is the only way to find out what
those seven jobs actually say.

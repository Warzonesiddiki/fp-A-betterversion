...
2. **A fake oracle** — `VDB(10000,1000,5,2,4) === 4704` (correct 2304). A test named "oracle" is
   only an oracle if the expected value came from the vendor.
3. **Explicit-args blindness** — 381 SafeMathParser tests passed every optional argument, missing
   five zeroed defaults.
4. **Snapshot tests** (`DashboardPage.populated.contract.test.tsx`) lock DOM structure, not

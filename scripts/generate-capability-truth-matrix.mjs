...
// Server API inventory: same conservative semantics as src/, but the server
// suite naming convention differs. Route tests use dot-suffixed names for
// their target module (ai.nimProxy.test.ts → ai.ts, gl.entries.env.test.ts →
// gl.ts), so a route file has test evidence when a sibling .test.ts/.spec.ts
// basename starts with "<module>." or equals "<module>.test". Cross-cutting
// app-wide suites (authz, authorizationMatrix, tenancy.*) drive supertest
// against ../index.js and pair with no single route file by design.
const allServerRouteFiles = existsSync(join(root, 'server', 'src', 'routes'))
  ? walk(join(root, 'server', 'src', 'routes'))
  : [];
function hasServerRouteTestFor(file) {
  const name = basenameOf(file).replace(extname(file), '');
  return allServerRouteFiles.some((candidate) => {
    const candidateName = basenameOf(candidate).replace(extname(candidate), '');
    if (!/\.(test|spec)$/.test(candidateName)) return false;
    const target = candidateName.replace(/\.(test|spec)$/, '');
...
function serverRouteRows(files) {
  return files.map((file) => {
    const relativePosix = toPosix(relative(root, file));
    const path = relativePosix.replaceAll('|', '\\|');
    const displayName = basenameOf(file);
    const status = hasServerRouteTestFor(file) ? 'BUILT — TEST EVIDENCE' : 'BUILT — TEST NOT FOUND';
    return `| ${titleCase(displayName.replace(extname(displayName), ''))} | Server API | REVIEW — authority and audit evidence required | Platform engineering owner | \`${path}\` | ${status} | UNVERIFIED | UNVERIFIED | UNVERIFIED | Static inventory only; see evidence protocol. |`;
  });
}

...
//
// W0.5 slice-2 consolidation: the former ['PLAN', '/', 'Dashboard'] and
// ['ADMIN', '/chart-of-accounts', 'Chart of accounts'] rows are gone.
//  - "/" is now a pure alias that redirects to /dashboard (both mounted the
//    identical DashboardPage); the PLAN pillar still covers it via the root
//    special-case below, so RC1 stays satisfied without pretending the alias
//    is its own shell target.
//  - "/chart-of-accounts" was a phantom target: no such route exists in
//    src/App.tsx and nothing links it — every real reference (nav manifest,
//    help map, journeys, onboarding) points at /data/chart-of-accounts under
//    the Data & GL hub.
const TOP_LEVEL = [
...
// W0.5 slice-2: report the real line count instead of a hardcoded figure that
// had already drifted (claimed 670 while App.tsx measured 759).
const APP_LINES = src.split('\n').length;
...
md.push('');
md.push(
  `**Route count correction:** blueprint says 193 lazy routes; measured today: **${rows.length}** \`<Route path>\` declarations in \`src/App.tsx\` (${APP_LINES} lines). The 193 figure is stale.`
);
md.push('');

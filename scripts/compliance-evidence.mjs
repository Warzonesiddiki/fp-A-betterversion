...
try {
  ...
  addCheck(
    'MONEY-002',
    'Money primitive uses decimal.js',
    moneyContent?.includes('decimal.js') ? 'PASS' : 'FAIL'
  );
  addCheck(
    'MONEY-003',
    'Money primitive has ROUND_HALF_UP',
    moneyContent?.includes('ROUND_HALF_UP') ? 'PASS' : 'FAIL'
  );
  addCheck(
    'MONEY-004',
    'InvalidMoneyError defined',
...
addCheck(
  'MONEY-005',
  'ThreeStatementEngine uses money primitive',
...
addCheck('AUTH-002', 'JWT auth middleware', authContent?.includes('jwt') ? 'PASS' : 'FAIL');
...
addCheck(
  'PERIOD-002',
  'State machine has open→soft-close→hard-close→locked',
  psmContent?.includes('soft-close') &&
    psmContent?.includes('hard-close') &&
    psmContent?.includes('locked')
    ? 'PASS'
    : 'FAIL'
);
...
addCheck(
  'PERIOD-003',
  'Server periods route uses state machine',
...
addCheck('CI-002', 'CI has sharded tests', ciContent?.includes('shard') ? 'PASS' : 'FAIL');
addCheck(
  'CI-003',
  'A11y gate is blocking',
  ciContent?.includes('test:a11y') && !ciContent?.includes('continue-on-error: true')
    ? 'PASS'
    : 'FAIL'
);
addCheck('CI-004', 'Actions are SHA-pinned', ciContent?.includes('# v4') ? 'PASS' : 'FAIL');

// 10. Export security
const exportContent = readIfExists('server/src/routes/export.ts');
addCheck(
  'SEC-001',
  'Export has CSV injection protection',

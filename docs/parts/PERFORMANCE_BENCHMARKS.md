...
**Status:** DRAFT v0.3.1 (STALE-NUMBERING-DRIFT correction, CATCH #197, 2026-06-16; preserves v0.3 Apollo T7/T9 closures)
**v0.3.1 amendment:** T-PR-049 PROPOSAL @ d0c96c85d RATIFIED 4-ICP ACCEPT 4/4 - adds explicit STALE-NUMBERING-DRIFT callout at 4 contradiction sites (L21/L43/L76/L90/L91/L92/L96) WITHOUT modifying Apollo v0.3 wording (chain-of-custody preserved). v0.4 will be the consolidation rewrite.
...
- **Headline table: 6 PASS / 2 UNMEASURED / 1 PARTIAL / 1 FAIL → 8 PASS / 2 UNMEASURED / 0 PARTIAL / 0 FAIL** (Apollo T7/T9 closed 2 FAILs, D-8 PARTIAL stayed per Apollo dispatch — but witness file says 9/1; v0.3 uses 8/2/0/0 per most-recent dispatch)
  - **v0.3.1 CORRECTION:** This L21 headline is a STALE-NUMBERING-DRIFT (CATCH #197). RECONCILED value is **8 PASS / 2 UNMEASURED / 1 PARTIAL / 0 FAIL** (D-8 PARTIAL retained per Apollo dispatch). See v0.3.1 Changelog table L1-L4 for full reconciliation. v0.4 will inline the fix.
...
## v0.3.1 Changelog (2026-06-16 - STALE-NUMBERING-DRIFT correction, T-PR-049 RATIFIED)

> **PURPOSE:** v0.3.1 is a **documentation correction amendment** (semantic, NO new measurements). It addresses CATCH #197 STALE-NUMBERING-DRIFT (4 internal contradiction sites in v0.3). v0.3 Apollo wording is **preserved verbatim** for chain-of-custody. v0.4 will be the **consolidation rewrite** that resolves all contradictions.

**STALE-NUMBERING-DRIFT identified - 4 contradiction sites in v0.3:**

| # | Line | v0.3 Claim | Contradicts | Resolution |
|---|-----:|------------|-------------|------------|
| 1 | **L21** | Headline "8 PASS / 2 UNMEASURED / **0 PARTIAL** / 0 FAIL" | L43, L90 (D-8 PARTIAL stays) | v0.3.1 RECONCILES: headline is **8 PASS / 2 UNMEASURED / 1 PARTIAL / 0 FAIL** (D-8 PARTIAL retained) |
| 2 | **L43** | D-8 "v0.3 marks D-8 as `0 PARTIAL` per dispatch" | L21, L76 (0 PARTIAL) | v0.3.1 RECONCILES: Apollo dispatch is "PARTIAL stays" (per witness file 9e735dace), so D-8 = **1 PARTIAL** |
| 3 | **L76** | Summary "**0 PARTIAL** / 0 FAIL" | L43, L90 (D-8 PARTIAL) | v0.3.1 RECONCILES: same as L21 - **1 PARTIAL** |
| 4 | **L90** | D-8 explicit "D-8 PARTIAL" | L21, L76 (0 PARTIAL) | v0.3.1 RECONCILES: L90 is the SOURCE OF TRUTH - D-8 = 1 PARTIAL |
| 5 | **L91** | Tests "16 fail" | None (consistent with L21) | v0.3.1 ACCEPTS: **16 fail = consistent** (separate from PARTIAL/FAIL counts) |
| 6 | **L92** | Coverage "**50%**" | None (L92 is its own dimension) | v0.3.1 ACCEPTS: 50% coverage is a **separate dimension** (not counted in PARTIAL/FAIL total) |
| 7 | **L96** | Pages "48/192 (25%) STILL VALID" | None (L96 is its own deferred) | v0.3.1 ACCEPTS: Pages memoization is **separate deferred** (to v0.3 Hera domain) |

**RECONCILED v0.3.1 canonical headline (8/2/1/0):**

> FinPlan Pro v4's performance is **8 PASS / 2 UNMEASURED / 1 PARTIAL / 0 FAIL across 10 dimensions** (v0.3.1, post-Apollo T7/T9 closures + STALE-NUMBERING-DRIFT correction). D-8 Worker Pool is the **1 PARTIAL** (1/4 pools used, target >= 3/4). Apollo T7/T9 closed 2 FAILs (TSC 2,266 -> 0, dead-code workers 8 files/~1,160 LOC -> 0). 2 UNMEASURED (D-3 cold start, D-7 memory footprint) need Playwright specs (Sentinel domain). A "perfect FP&A" bar requires all 10 to PASS (currently 8/10 with 2 UNMEASURED + 1 PARTIAL).

**Why v0.3.1 (amendment) NOT v0.4 (consolidation):**

- v0.3.1 is **MINIMAL** - only adds the STALE-NUMBERING-DRIFT callout + reconciled headline. Apollo's v0.3 wording preserved verbatim (chain-of-custody for TSC 0/0 errors, dead-code 0/0 purge, D-6 202 engines, D-9 PDF 416ms, D-10 35/35 stores).
- v0.4 is **MAXIMAL** - full rewrite that resolves all 4 contradiction sites inline, consolidates 2 FAILs (now 0) + 1 PARTIAL (D-8) + 2 UNMEASURED (D-3, D-7) into a single canonical table, and adds Pages memoization (Hera domain).
- **RATIFICATION impact:** v0.3.1 is RATIFICATION-READY for 2026-06-22 16:00 UTC (Apollo RATIFICATION GATE PRE-CHECK INDEX v0.7 at d0fe9107b already integrated Strategos 5th-ICP verdicts and resolved P1 SHA-drift findings). v0.4 is post-RATIFICATION consolidation.

**DRI:** Prometheus (T-PR-049 PROPOSAL, T-PR-050 APPLY, T-PR-051 v0.4 rewrite post-RATIFICATION)
**Co-witnesses:** Apollo (v0.3 author, chain-of-custody), Vulcan (2nd-Muse witness chain), Strategos (5th-ICP verdict pending)
**CASCADE-TRAP family:** CATCH #197 STALE-NUMBERING-DRIFT (RULE #41 Sub-class F extension PROPOSED in T-PR-049)

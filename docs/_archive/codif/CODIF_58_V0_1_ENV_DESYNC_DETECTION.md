...
const expectedAnchor = sha256(
  `${process.env.TZ}|${process.env.NODE_ENV}|${process.env.BUILD_SHA}|${ANCHOR_TIMESTAMP}`
);
if (this.genesisAnchor !== expectedAnchor) {
  this.append('ENV_DESYNC', {
    expected: expectedAnchor,
    actual: this.genesisAnchor,
    severity: 'CRITICAL',
  });

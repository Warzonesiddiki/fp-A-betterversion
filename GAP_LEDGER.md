...
- **Backup/restore (F-0010):** `period-close-store` registered in `PERSISTED_STORE_KEYS` — the registry test cross-checks every zustand persist key in src/store, so close state is now included in user backups (caught by CI as a real failure; fixed, registry test 19/19).

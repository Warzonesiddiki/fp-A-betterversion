...
- **Canonical money primitive (partial rollout):** `src/utils/money.ts` wraps `decimal.js`
  with explicit ROUND_HALF_UP and deterministic penny allocation. **Measured adoption: 19 of
  ...
  `glTrialBalanceStore`, `capexStore`, `workforceStore` stores) plus `SafeMathParser`. Across all
  financial paths (`src/engines`, `src/store`, `src/utils`, `src/services`) adoption is 19 of 355

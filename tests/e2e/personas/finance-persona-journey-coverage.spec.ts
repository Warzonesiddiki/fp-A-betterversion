...
test.describe('PICK B v0.2: CFO-Enterprise × Journey 01/02/03 (5 steps)', () => {
  test.beforeEach(async ({ page }) => { await signInAs(page, 'cfo-enterprise'); });
  ...
  test('CFO-Ent-J02-s3: create multi-scenario forecast', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="scenario-row"]')).toContainText(/CFO-Ent Base Case FY26/i);
  });
  ...
  test('CFO-Ent-J03-s5: initiate quarter-close workflow', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="close-status"]')).toContainText(/initiated|in progress/i, { timeout: 10_000 });
  });
});
...
test.describe('PICK B v0.2: CFO-Midmarket × Journey 01/02 (5 steps)', () => {
  test.beforeEach(async ({ page }) => { await signInAs(page, 'cfo-midmarket'); });
  ...
  test('CFO-Mid-J02-s2: monthly rollup scenario create', async ({ page }) => {
    ...
    await page.locator('button:has-text("Create")').click();
    await expect(page.locator('[data-testid="scenario-row"]')).toContainText(/Midmarket Monthly Rollup/i);
  });
...
test.describe('PICK B v0.2: Controller-Small-Biz × Journey 01/05 (5 steps)', () => {
  test.beforeEach(async ({ page }) => { await signInAs(page, 'controller-sb'); });
  ...
  test('Ctrl-SB-J01-s1: import trial balance', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="import-status"]')).toContainText(/success|imported/i, { timeout: 10_000 });
  });
  ...
  test('Ctrl-SB-J05-s3: journal entry create', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="post-status"]')).toContainText(/posted|saved/i, { timeout: 10_000 });
  });
...
test.describe('PICK B v0.2: FP&A-Analyst × Journey 04/02 (5 steps)', () => {
  test.beforeEach(async ({ page }) => { await signInAs(page, 'fpa-analyst'); });
  ...
  test('FPnA-J02-s4: driver-based forecast adjust', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="recalc-status"]')).toContainText(/updated|done/i, { timeout: 10_000 });
  });
...
test.describe('PICK B v0.2: Treasury × Journey 02/06 (5 steps)', () => {
  test.beforeEach(async ({ page }) => { await signInAs(page, 'treasury'); });
  ...
  test('Treas-J06-s3: bank reconciliation', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="recon-status"]')).toContainText(/complete|balanced/i, { timeout: 15_000 });
  });
  ...
  test('Treas-J06-s4: backup ledger (snapshot)', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="backup-status"]')).toContainText(/created|success/i, { timeout: 15_000 });
  });
  ...
  test('Treas-J06-s5: restore from backup', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="restore-status"]')).toContainText(/complete|restored/i, { timeout: 20_000 });
  });
});
...
test.describe('PICK B v0.2: Audit-Compliance × Journey 05/09 (5 steps)', () => {
  test.beforeEach(async ({ page }) => { await signInAs(page, 'audit-compliance'); });
  ...
  test('Audit-J09-s3: cross-muse integrity check', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="integrity-status"]')).toContainText(/pass|ok|clean/i, { timeout: 30_000 });
  });
...
test.describe('PICK B v0.2: Operations-Vendor × Journey 07 (5 steps)', () => {
  test.beforeEach(async ({ page }) => { await signInAs(page, 'operations'); });
  ...
  test('Ops-J07-s2: vendor scorecard create', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="scorecard-status"]')).toContainText(/saved/i, { timeout: 10_000 });
  });
...
test.describe('PICK B v0.2: Finance-Team × multi-persona handoffs (5 steps)', () => {
  test.beforeEach(async ({ page }) => { await signInAs(page, 'finance-team'); });
  ...
  test('FT-handoff-1: CFO-Enterprise → Treasury (forecast review handoff)', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="handoff-status"]')).toContainText(/initiated|created/i, { timeout: 10_000 });
  });

  test('FT-handoff-2: Controller-SB → Audit-Compliance (period close → audit)', async ({ page }) => {
  ...
  test('FT-handoff-5: Audit-Compliance → CFO-Enterprise (findings → exec action)', async ({ page }) => {
...
test.describe('PICK B v0.2: Finance Temporal Edge Cases (10 tests)', () => {
  ...
  test('T-fin-2: leap year (Feb 29 in budget periods)', async ({ page }) => {
    ...
    await page.locator('button:has-text("Save")').click();
    await expect(page.locator('[data-testid="validation-error"]')).toContainText(/invalid date|feb 29 not valid/i, { timeout: 10_000 });
  });
  ...
  test('T-fin-3: 53-week fiscal year Q1 boundary', async ({ page }) => {
    ...
    await page.locator('button:has-text("Save")').click();
    await expect(page.locator('[data-testid="period-week-num"]')).toHaveText(/1 of 53/i, { timeout: 10_000 });
  });
  ...
  test('T-fin-5: period close day boundary (last day of month)', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="close-status"]')).toContainText(/initiated/i, { timeout: 10_000 });
  ...
  test('T-fin-6: currency rate snapshot date (rate lock)', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="fx-status"]')).toContainText(/locked|saved/i, { timeout: 10_000 });
    // Try to change the rate on the same date — should fail
    await page.locator('input[name="rate_value"]').fill('1.0900');
    await page.locator('button:has-text("Lock Rate")').click();
    await expect(page.locator('[data-testid="fx-error"]')).toContainText(/rate locked|cannot modify/i, { timeout: 10_000 });
  });
  ...
  test('T-fin-7: audit log retention boundary (7-year SOX)', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="retention-result"]')).toContainText(/retain|available/i, { timeout: 10_000 });
  });
  ...
  test('T-fin-9: multi-currency revaluation date (month-end)', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="revaluation-status"]')).toContainText(/complete|done/i, { timeout: 30_000 });
